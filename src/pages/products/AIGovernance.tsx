import { Card, CardContent } from "@/components/ui/card";
import { Link } from "react-router-dom";
import { ArrowLeft, Bot, Workflow, MessageSquare, FileText, Zap, Shield } from "lucide-react";
import aiGovernanceChatbot from "@/assets/products/ai-governance-chatbot.jpg";
import aiGovernanceWhatsApp from "@/assets/products/ai-governance-whatsapp.jpg";
import aiGovernanceServices from "@/assets/products/ai-governance-services.jpg";
import ProductDemoForm from "@/components/ProductDemoForm";

const AIGovernance = () => {
  const features = [
    {
      icon: Bot,
      title: "Agentic AI System",
      description: "Autonomous AI agents that understand, route, and resolve grievances intelligently"
    },
    {
      icon: Workflow,
      title: "Intelligent Routing",
      description: "Automatic classification and routing of complaints to the right department and officer"
    },
    {
      icon: MessageSquare,
      title: "WhatsApp Integration",
      description: "Citizens can file and track complaints via WhatsApp for maximum accessibility"
    },
    {
      icon: FileText,
      title: "Auto-Documentation",
      description: "Automatic generation of reports, summaries, and documentation from conversations"
    },
    {
      icon: Zap,
      title: "Quick Resolution",
      description: "AI-suggested solutions and automated follow-ups to accelerate resolution times"
    },
    {
      icon: Shield,
      title: "Multi-Language Support",
      description: "Natural language processing in multiple regional languages for inclusive governance"
    }
  ];

  const benefits = [
    "Reduced response time for citizen grievances",
    "Lower administrative burden on government staff",
    "Improved citizen satisfaction and trust",
    "Better tracking and accountability of complaints",
    "Data-driven insights for policy improvements",
    "24/7 availability for citizen services"
  ];

  return (
    <div className="min-h-screen pt-20">
      {/* Hero Section */}
      <section className="py-12 md:py-16 hero-gradient text-white relative overflow-hidden">
        <div className="absolute inset-0 opacity-10">
          <div className="absolute top-0 left-1/4 w-96 h-96 bg-white rounded-full blur-3xl"></div>
          <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-white rounded-full blur-3xl"></div>
        </div>
        <div className="container-custom relative z-10">
          <Link to="/products" className="inline-flex items-center text-white/90 hover:text-white mb-6 transition-colors">
            <ArrowLeft className="mr-2" size={20} />
            Back to Products
          </Link>
          <h1 className="text-4xl md:text-5xl font-bold mb-4">Agentic AI Governance Platform</h1>
          <p className="text-xl text-white/90 max-w-3xl">
            Intelligent Grievance Management with AI Agents
          </p>
        </div>
      </section>

      {/* Breadcrumb */}
      <section className="bg-gray-100 py-4">
        <div className="container-custom">
          <div className="text-sm text-gray-600">
            <span>Home</span> <span className="mx-2">/</span>
            <span>Products</span> <span className="mx-2">/</span>
            <span className="text-primary font-semibold">AI Governance Platform</span>
          </div>
        </div>
      </section>

      {/* Product Overview */}
      <section className="py-10 md:py-12">
        <div className="container-custom">
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div>
              <h2 className="text-3xl font-bold mb-4">AI-Powered Citizen Services</h2>
              <p className="text-gray-700 mb-4">
                The Agentic AI Governance Platform revolutionizes how governments handle citizen grievances by deploying intelligent AI agents that can understand, classify, route, and even resolve complaints autonomously.
              </p>
              <p className="text-gray-700 mb-4">
                Using advanced natural language processing and machine learning, the platform understands citizen concerns in multiple languages, automatically routes them to appropriate departments, suggests solutions, and generates comprehensive documentation.
              </p>
              <p className="text-gray-700">
                Integrated with popular communication channels like WhatsApp, the platform makes government services accessible to every citizen, regardless of their digital literacy or location.
              </p>
            </div>
            <div className="overflow-hidden rounded-lg shadow-lg">
              <img src={aiGovernanceServices} alt="AI-Powered Citizen Services" className="w-full h-full object-cover" />
            </div>
          </div>
        </div>
      </section>

      {/* Product Screenshots */}
      <section className="py-10 md:py-12 bg-gray-50">
        <div className="container-custom">
          <h2 className="text-3xl font-bold text-center mb-8">AI-Powered Interfaces</h2>
          <div className="grid md:grid-cols-2 gap-8">
            <div className="overflow-hidden rounded-lg shadow-lg">
              <img src={aiGovernanceChatbot} alt="AI Governance Chatbot Interface" className="w-full h-full object-cover blur-sm" />
              <div className="bg-white p-4">
                <h3 className="font-semibold text-lg blur-sm">AI Chatbot Dashboard</h3>
                <p className="text-gray-600 text-sm blur-sm">Intelligent grievance routing and resolution system</p>
              </div>
            </div>
            <div className="overflow-hidden rounded-lg shadow-lg">
              <img src={aiGovernanceWhatsApp} alt="WhatsApp Integration" className="w-full h-full object-cover blur-sm" />
              <div className="bg-white p-4">
                <h3 className="font-semibold text-lg blur-sm">WhatsApp Integration</h3>
                <p className="text-gray-600 text-sm blur-sm">Citizens can file complaints via WhatsApp</p>
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* Key Features */}
      <section className="py-10 md:py-12 bg-gray-50">
        <div className="container-custom">
          <h2 className="text-3xl font-bold text-center mb-8">Key Features</h2>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((feature, index) => (
              <Card key={index} className="card-hover">
                <CardContent className="p-6">
                  <feature.icon className="text-primary mb-4" size={40} />
                  <h3 className="text-xl font-semibold mb-2">{feature.title}</h3>
                  <p className="text-gray-700">{feature.description}</p>
                </CardContent>
              </Card>
            ))}
          </div>
        </div>
      </section>

      {/* Benefits */}
      <section className="py-10 md:py-12">
        <div className="container-custom">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-3xl font-bold text-center mb-8">Benefits</h2>
            <Card>
              <CardContent className="p-8">
                <ul className="space-y-3">
                  {benefits.map((benefit, index) => (
                    <li key={index} className="flex items-start">
                      <span className="text-primary mr-3 mt-1">✓</span>
                      <span className="text-gray-700">{benefit}</span>
                    </li>
                  ))}
                </ul>
              </CardContent>
            </Card>
          </div>
        </div>
      </section>

      {/* Demo Request Form */}
      <section className="py-10 md:py-12">
        <div className="container-custom">
          <ProductDemoForm productName="Agentic AI Governance Platform" />
        </div>
      </section>

      {/* CTA Section */}
      <section className="py-12 md:py-16 hero-gradient text-white relative overflow-hidden">
        <div className="absolute inset-0 opacity-10">
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-white rounded-full blur-3xl"></div>
        </div>
        <div className="container-custom text-center relative z-10">
          <h2 className="text-3xl md:text-4xl font-bold mb-6">
            Transform Citizen Services with AI
          </h2>
          <p className="text-xl mb-8 text-white/90 max-w-2xl mx-auto">
            Learn how AI agents can revolutionize your governance systems
          </p>
          <Link to="/contact">
            <button className="bg-white text-primary hover:bg-white/90 px-8 py-4 rounded-lg font-semibold text-lg shadow-xl transition-all duration-300 hover:shadow-2xl">
              Schedule a Demo
            </button>
          </Link>
        </div>
      </section>
    </div>
  );
};

export default AIGovernance;
